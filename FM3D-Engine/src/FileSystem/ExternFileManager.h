...
namespace ENGINE_NAME {
	...
	public:
		...
		static bool ReadTextureFile(const char* filename, Texture* texture, Texture::FilterMode filterMode = Texture::LINEAR, Texture::WrapMode wrapMode = Texture::CLAMP);
		static bool ReadFontFile(const char* filename, uint size, Vector2f scale, Texture* inTexture, Font** result);
		static void ReadModelFile(const char* filename, RenderSystem* renderSystem, Model** model, bool supportsInstancing, bool useAnimation = true);
