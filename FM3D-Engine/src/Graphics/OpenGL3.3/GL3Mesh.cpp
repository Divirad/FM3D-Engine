...
namespace FM3D {
	...
	GL3Mesh::GL3Mesh(const Skeleton* skeleton, bool supportsInstancing, const std::vector<MeshPart>& parts): Mesh(skeleton, supportsInstancing, parts.size()), m_glBuffers(parts.size()) {
		bool isAnimated = skeleton != nullptr;

		for (uint p = 0; p < parts.size(); p++) {
			const MeshPart& part = parts[p];
