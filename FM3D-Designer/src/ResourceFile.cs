...
public:
	...
	void LoadResources(FM3D::RenderSystem* renderSystem);
        ...
        private const string HEADER_CPP = @"//
//	DO NOT TOUCH THIS!!!
//	AUTOGENERATED FILE BY FM3D-DESIGNER!!!
//
#include ""Resources.h""
...
void Resources::LoadResources(FM3D::RenderSystem* renderSystem) {";
        ...
        public bool WriteFile()
        {
            try
            {
                ...
                if (!File.Exists(Path_CPP)) File.Create(Path_CPP);
                using (FileStream fstream = new FileStream(Path_CPP, FileMode.Truncate, FileAccess.Write, FileShare.Read))
                {
                    using (StreamWriter file = new StreamWriter(fstream))
                    {
                        file.AutoFlush = true;
                        ...
                        if (Item.AllItems.ContainsKey(ItemTypes.MeshFile))
                        {
                            foreach (var f in Item.AllItems[ItemTypes.MeshFile])
                            {
                                file.WriteLine("m_meshes.insert(FM3D::FileManager::ReadMeshFile(\"" + f.Path.Replace('\\', '/') + "\", renderSystem, m_skeleton));");
                            }
                        }
                        file.WriteLine("}");
                    }
